...
<div class="max-w-6xl mx-auto print-container">
    ...
    <div class="flex flex-col md:flex-row justify-between items-start md:items-center mb-8 gap-6">
        <div class="space-y-1">
            <div class="flex items-center gap-2 text-primary font-bold text-xs tracking-widest uppercase">
            ...
            <div class="text-slate-500 flex items-center gap-4 text-sm mt-2">
                <p class="flex items-center gap-2">
                    <span class="material-symbols-outlined text-sm">person</span>
                    Nama: <?= esc($insertData['nama_user'] ?? 'Pengguna Anonim') ?>
                </p>
                <p class="flex items-center gap-2">
    ...
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                ...
                <div class="p-6 md:p-8 border-b border-slate-100 bg-gradient-to-br from-primary/5 via-white to-white relative">
                    ...
                    <div class="flex flex-col md:flex-row gap-6 justify-between items-start relative z-10">
                        <div class="flex-1">
                            ...
                            <h3 class="text-3xl font-black text-slate-900 mb-1 leading-tight"><?= esc($utama['penyakit']['nama_penyakit']) ?></h3>
                            <p class="text-slate-400 text-sm mb-4"><?= esc($utama['penyakit']['kode_penyakit']) ?></p>
                    ...
                    <div class="mt-8 bg-slate-50/80 backdrop-blur-sm p-5 rounded-2xl border border-slate-100 group transition-all hover:bg-slate-50">
                        ...
                        <p class="text-slate-600 text-sm leading-relaxed"><?= esc($utama['penyakit']['solusi'] ?? '-') ?></p>
                    </div>
                </div>
                ...
                <div class="p-6 md:p-8 bg-white border-b border-slate-100">
                    ...
                    <div class="space-y-4">
                        <div class="rounded-2xl border border-red-500/10 bg-red-50/50 p-6 shadow-sm">
                            ...
                            <p class="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap ml-11"><?= esc($utama['penyakit']['tindakan_segera'] ?? 'Belum ada panduan tindakan segera.') ?></p>
                        </div>
                        ...
                        <div class="rounded-2xl border border-blue-500/10 bg-blue-50/50 p-6 shadow-sm">
                            ...
                            <p class="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap ml-11"><?= esc($utama['penyakit']['protokol_pengobatan'] ?? 'Belum ada panduan protokol pengobatan.') ?></p>
                        </div>
                        ...
                        <div class="rounded-2xl border border-green-500/10 bg-green-50/50 p-6 shadow-sm">
                            ...
                            <p class="text-slate-700 text-sm leading-relaxed whitespace-pre-wrap ml-11"><?= esc($utama['penyakit']['strategi_pencegahan'] ?? 'Belum ada panduan strategi pencegahan.') ?></p>
                        </div>
                    </div>
                </div>
                            ...
                            <div class="p-5">
                                <div class="grid grid-cols-3 gap-4 text-center">
                                    <div class="bg-slate-50 p-3 rounded-xl">
                                        ...
                                        <div class="text-lg font-black text-slate-800"><?= number_format($dp['cf_user'], 2) ?></div>
                                    </div>
                                    <div class="bg-slate-50 p-3 rounded-xl">
                                        <div class="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">CF Pakar</div>
                                        <div class="text-lg font-black text-slate-800"><?= number_format($dp['cf_pakar'], 2) ?></div>
                                    </div>
                                    <div class="bg-primary/10 p-3 rounded-xl">
                                        <div class="text-[10px] uppercase tracking-widest text-primary font-bold mb-1">CF Hasil</div>
                                        <div class="text-lg font-black text-primary"><?= number_format($dp['cf_hasil'], 4) ?></div>
                                    </div>
                                </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-8">
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                ...
                <ul class="space-y-3">
                    ...
                    <li class="flex items-start gap-3 p-3 rounded-2xl bg-slate-50 border border-slate-100 transition-all hover:bg-white hover:shadow-md hover:border-primary/20 group">
                        ...
                        <span class="text-sm text-slate-600 leading-snug font-medium"><?= esc($nama) ?></span>
                    </li>
                    <?php endforeach; ?>
                </ul>
            </div>
            ...
            <div class="bg-white rounded-3xl border border-slate-200 p-6 shadow-sm">
                <div class="flex items-center justify-between mb-5">
                    <h3 class="text-lg font-black text-slate-900">Diferensial</h3>
                    <span class="text-[10px] font-black text-slate-400 uppercase tracking-widest">Alternatif</span>
                </div>
                <div class="flex flex-col gap-3">
                    <?php foreach (array_slice($hasil, 1) as $h):
                        ...
                        $hGambarSrc = (!empty($hGambar) && (strpos($hGambar, 'http') === 0 || strpos($hGambar, 'uploads/') === 0))
                            ...
                            : '';
                    ?>
                    <div class="relative group p-4 rounded-2xl border border-slate-100 bg-slate-50/50 hover:bg-white hover:border-primary/30 transition-all cursor-pointer hover:shadow-lg hover:shadow-slate-200/50">
                        <div class="flex items-center gap-3 mb-2">
                            <?php if (!empty($hGambarSrc)): ?>
                            <div class="size-10 rounded-lg overflow-hidden border border-slate-200 flex-shrink-0">
                            ...
                            <div class="size-10 rounded-lg bg-primary/10 flex items-center justify-center flex-shrink-0">
                                <span class="material-symbols-outlined text-primary text-lg">coronavirus</span>
                            </div>
                            <?php endif; ?>
                            <div class="flex-1 min-w-0">
                                <div class="flex justify-between items-center mb-1">
                                    <span class="font-bold text-xs text-slate-700 truncate"><?= esc($h['penyakit']['nama_penyakit']) ?></span>
                                    <span class="text-[10px] font-black text-primary bg-primary/10 px-2 py-0.5 rounded-full ml-2 flex-shrink-0"><?= $h['persentase'] ?>%</span>
                                </div>
