                ...
                <div class="p-6 md:p-8 border-b border-slate-100 bg-gradient-to-br from-primary/5 via-white to-white relative">
                    ...
                    <div class="flex flex-col md:flex-row gap-6 justify-between items-start relative z-10">
                        <div class="flex-1">
                            ...
                            <h3 class="text-xl font-bold text-slate-900 mb-1"><?= esc($riwayat['nama_penyakit']) ?></h3>
                            <p class="text-slate-400 text-xs mb-3"><?= esc($riwayat['kode_penyakit']) ?></p>
                    ...
                    <div class="mt-4 bg-slate-50/80 backdrop-blur-sm p-4 rounded-xl border border-slate-100 group transition-all hover:bg-slate-50">
                        ...
                        <p class="text-slate-600 text-xs leading-relaxed"><?= esc($riwayat['solusi_penyakit'] ?? '-') ?></p>
                    </div>
                </div>
                ...
                <div class="p-4 md:p-6 bg-white border-b border-slate-100">
                    ...
                    <div class="space-y-3">
                        <div class="rounded-xl border border-red-500/10 bg-red-50/50 p-4 shadow-sm">
                            ...
                            <p class="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap ml-8"><?= esc($riwayat['tindakan_segera'] ?? 'Belum ada panduan tindakan segera.') ?></p>
                        </div>
                        ...
                        <div class="rounded-xl border border-blue-500/10 bg-blue-50/50 p-4 shadow-sm">
                            ...
                            <p class="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap ml-8"><?= esc($riwayat['protokol_pengobatan'] ?? 'Belum ada panduan protokol pengobatan.') ?></p>
                        </div>
                        ...
                        <div class="rounded-xl border border-green-500/10 bg-green-50/50 p-4 shadow-sm">
                            ...
                            <p class="text-slate-700 text-xs leading-relaxed whitespace-pre-wrap ml-8"><?= esc($riwayat['strategi_pencegahan'] ?? 'Belum ada panduan strategi pencegahan.') ?></p>
                        </div>
                    </div>
                </div>
...
<!-- Detail Perhitungan CF -->
                <div class="p-4 md:p-6 bg-white">
                    ...
                    <div class="space-y-3">
                            ...
                            <div class="p-4">
                                <div class="grid grid-cols-3 gap-3 text-center">
                                    <div class="bg-slate-50 p-2 rounded-lg">
                                        ...
                                        <div class="text-sm font-bold text-slate-800"><?= number_format(floatval($dp['cf_user'] ?? 0), 2) ?></div>
                                    </div>
                                    <div class="bg-slate-50 p-2 rounded-lg">
                                        <div class="text-[10px] uppercase tracking-widest text-slate-400 font-bold mb-1">CF Pakar</div>
                                        <div class="text-sm font-bold text-slate-800"><?= number_format(floatval($dp['cf_pakar'] ?? 0), 2) ?></div>
                                    </div>
                                    <div class="bg-primary/10 p-2 rounded-lg">
                                        <div class="text-[10px] uppercase tracking-widest text-primary font-bold mb-1">CF Hasil</div>
                                        <div class="text-sm font-bold text-primary"><?= number_format(floatval($dp['cf_hasil'] ?? 0), 4) ?></div>
                                    </div>
                                </div>
                        ...
                        <!-- CF Combine Result -->
                        <div class="mt-4 p-4 rounded-xl bg-slate-900 text-white relative overflow-hidden">
                            ...
                            <div class="flex items-start gap-3 relative z-10">
                                ...
                                <div class="space-y-1">
                                    ...
                                    <p class="text-[10px] text-slate-400">CF Combine = CF₁ + CF₂ × (1 − CF₁)</p>
                                    <p class="text-lg font-bold mt-1"><?= number_format($cf_hasil, 4) ?> <span class="text-xs text-slate-400">(<?= number_format($persen, 2) ?>%)</span></p>
                                </div>
                            </div>
                        </div>
                    </div>
                    <?php else: ?>
                    <p class="text-slate-400 text-xs">Data perhitungan CF tidak tersedia.</p>
                    <?php endif; ?>
                </div>
            </div>
        </div>
        ...
        <div class="lg:col-span-4 flex flex-col gap-6">
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm">
                ...
                <ul class="space-y-2">
                    ...
                    <li class="flex items-start gap-2 p-2 rounded-xl bg-slate-50 border border-slate-100 transition-all hover:bg-white hover:shadow-sm hover:border-primary/20 group">
                        ...
                        <span class="text-xs text-slate-600 leading-snug font-medium"><?= esc($g['kode'] ?? '') ?> — <?= esc($g['nama'] ?? '') ?></span>
                    </li>
                    <?php endforeach; ?>
                </ul>
                <?php else: ?>
                <p class="text-slate-400 text-xs">Tidak ada data gejala tersimpan.</p>
                <?php endif; ?>
            </div>
            ...
            <div class="bg-white rounded-xl border border-slate-200 p-4 shadow-sm no-print">
                ...
                <p class="text-slate-400 text-xs mb-3">Tindakan ini bersifat permanen.</p>
                <a href="<?= base_url('riwayat/hapus/' . $riwayat['id_riwayat']) ?>"
                   onclick="return confirm('Apakah Anda yakin?')"
                   class="block text-center w-full py-2 bg-red-500 text-white font-semibold rounded-lg text-xs hover:bg-red-600 transition-all shadow-md">
                    Hapus Riwayat Ini
                </a>
            </div>
            <?php endif; ?>
