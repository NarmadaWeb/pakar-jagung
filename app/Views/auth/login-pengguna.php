...
<?= $this->section('meta') ?>
<meta name="description" content="Login ke sistem CornAI.">
<?= $this->endSection() ?>
...
<div class="w-full max-w-md px-4 py-8">
    <div class="bg-white dark:bg-slate-800 rounded-2xl shadow-xl p-8 border border-primary/10">
        <div class="text-center mb-8">
            ...
            <h1 class="text-2xl font-bold text-slate-900 dark:text-white">Login Pengguna</h1>
            <p class="text-slate-600 dark:text-slate-400 mt-2">Masuk untuk melakukan deteksi penyakit jagung</p>
        </div>
        ...
        <form action="<?= base_url('auth/proses-login') ?>" method="POST" class="space-y-6">
        ...
        <div class="mt-6 text-center">
            <p class="text-slate-600 dark:text-slate-400 text-sm">
                Belum punya akun?
                <a href="<?= base_url('auth/register') ?>" class="text-primary hover:text-primary/80 font-medium">Daftar Sekarang</a>
            </p>
        </div>

        <div class="mt-4 text-center">
            <a href="<?= base_url('login') ?>" class="text-slate-600 dark:text-slate-400 hover:text-primary text-sm">Login sebagai Admin</a>
